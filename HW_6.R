...
plot(cv.ridge)
coef(cv.ridge)
ridge.mse <- cv.ridge$cvm[cv.ridge$lambda == cv.ridge$lambda.1se]
...
plot(cv.ridge)
coef(cv.ridge)
lasso.mse <- cv.lasso$cvm[cv.lasso$lambda == cv.lasso$lambda.1se]

# PCR
pcr.fit <- pcr(crim ~ ., data = boston, scale = TRUE, validation = "CV")
summary(pcr.fit)  # 8 features best
pcr.mse <- 6.574^2

# Evaluate performance of models
barplot(c(subset.mse, ridge.mse, lasso.mse, pcr.mse), col = "blue",
        names.arg = c("best subset", "ridge", "lasso", "pcr"),
        main = "MSE for All Models")
